...
struct PlayerRuntime {
    Rectangle box{70, 270, 18, 46}; Vector2 velocity{}; Side facing{Side::Right}; CombatModel combat{};
...
void drawIrys(const Frame& frame, Rectangle box, Side facing, float motion, bool prime, bool anti = false, float damage = 0.0f) {
    ...
    if (frame.anatomy == Anatomy::AdditionalArm) limb({shoulder.x-dir*2,shoulder.y+2},{cx-dir*12,top+25+bob+swing*0.2f},skin);
...
private:
    ...
    int stageIndex_{}; int roomIndex_{}; float stageTime_{}; float runTime_{}; bool roomClear_{}; bool shutdown_{}; bool invulnerable_{}; bool debugDraw_{};
    ...
    void startStage(int stage,int room,bool unstable){
        ...
        spawnRoom();banner_=unstable?"SHUTDOWN RETURN":kStages[stage].name;bannerTime_=2.2f;
    }
    void spawnRoom(){
        enemies_.clear();projectiles_.clear();particles_.clear();roomClear_=false;
    ...
    void startBoss(int index){
        ...
        player_.box={70,kFloor-46,18,46};player_.velocity={};player_.combat.reset();banner_=toString(b.def.kind);bannerTime_=2.3f;
    }
    ...
    void updateGameplay(float dt,const InputFrame& in){
        ...
        updatePlayer(dt,in);for(auto& e:enemies_)updateEnemy(e,dt);if(boss_)updateBoss(*boss_,dt);updateProjectiles(dt);resolveCombat();
    ...
    void updatePlayer(float dt,const InputFrame& in){
        ...
        Rectangle previous=player_.box;player_.box.x+=player_.velocity.x*dt;player_.box.y+=player_.velocity.y*dt;player_.grounded=false;
    ...
    Rectangle swordVolume()const{
        const auto& cs=player_.combat.state();if(cs.authority!=Authority::NormalTechnique&&cs.authority!=Authority::PrimedTechnique)return{};float reach=selected_.reach+(cs.techniquePrimed?16:0);if(cs.technique==Technique::Advance)return{player_.techDirection>0?player_.box.x+player_.box.width:player_.box.x-reach,player_.box.y+8,reach,player_.box.height-12};
        if(cs.technique==Technique::Ascend)return{player_.box.x-9,player_.box.y-reach,player_.box.width+18,reach+20};return{player_.box.x-8,player_.box.y+player_.box.height-8,player_.box.width+16,reach};
    }
    ...
    void drawCandidateSelect(){title("ARK CANDIDATE CHAMBER // SELECT ONE // NO REROLLS");for(int i=0;i<3;i++)drawPod(70+i*171,110,candidates_[i],i==selectionIndex_);const auto&f=candidates_[selectionIndex_];std::string traits;for(std::size_t i=0;i<f.traits.size();++i){if(i)traits+="  /  ";traits+=f.traits[i];}DrawText(traits.c_str(),70,322,10,{216,125,196,255});DrawText("LEFT/RIGHT INSPECT // ENTER BIND ION",360,340,10,{94,117,148,255});}
    void drawFrameSelect(){title("PLANETARY-AI SUBSTRATES // RETAIN DAMAGE OR REPLACE COMPLETE FRAME");drawPod(25,110,selected_,selectionIndex_==0);DrawText("RETAIN CURRENT",43,96,12,{78,232,255,255});for(std::size_t i=0;i<replacements_.size();++i)drawPod(195+static_cast<float>(i)*145,110,replacements_[i],selectionIndex_==static_cast<int>(i)+1);DrawText("TRANSFER CLEARS PRIME AND ALL BODY STATE",25,332,10,{255,153,120,255});}
    void drawScene(){
    ...
    void drawFailure(){ClearBackground({12,9,15,255});for(int i=0;i<18;i++)DrawCircle(i*38,180+(i%3)*19,14,{35,25,39,255});title("SUBSTRATE LOST // ION RETURNED TO THE ARK");DrawText("The body remains. Ion rises. The route remembers you.",85,155,15,{206,191,205,255});DrawText("ENTER // PRESENT THREE NEW CANDIDATES",136,224,13,{78,233,255,255});DrawText("BACKSPACE // MAIN MENU",209,251,11,{113,123,149,255});}
    void drawEnding(){
